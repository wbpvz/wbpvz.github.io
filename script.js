...
const SOUND_THROTTLE = 500;
// Задержка между звуками 500мс
...
function initMobileIssueUI() {
    ...
    document.getElementById('m-phone-next-btn').onclick = () => {
        ...
        if (phone.length === 4) {
            ...
            alert('Некорректный код или номер телефона');
        }
    };
...
function renderMobileVerifyListModal() {
    ...
    remainingItems.forEach(item => {
        ...
        const codeClass = item.isAdult ? 'product-code-row adult-blur-text' : 'product-code-row';
...
function attachMainEventListeners() {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (detailsBtn) {
            ...
            if (code && currentOrder) {
                ...
                if (item && !item.isAdult) openProductDetailModal(item); // Не открываем модалку для 18+
            }
        }
    ...
    // Новые обработчики для курьера
    document.getElementById('courier-all-lost-finish-btn')?.addEventListener('click', () => {
        toggleModal('courier-all-lost-modal', false);
        finishOrderSession(currentOrder.identifier);
        showMainScreen();
    });
    document.getElementById('courier-some-lost-continue-btn')?.addEventListener('click', () => {
...
function handleReturnIdentifierInput(e) {
    ...
    clientCodeGroup.style.display = value.length === 4 ?
    'block' : 'none';
...
function openProductDetailModal(item) {
    ...
    `<img src="adult.svg" style="width:100%">` : (item.image ? `<img src="${item.image}" style="width:100%; object-fit:contain;">` : '📦');

    let receptionDateStr = item.date ?
    new Date(item.date).toLocaleString('ru-RU', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute:'2-digit' }) : "Неизвестно";
    const cellToDisplay = item.cell ?
    item.cell : currentOrder.cell;
    ...
    container.innerHTML = `
        <div class="pd-layout">
            ...
            <div class="pd-info-col">
                ...
                <div class="pd-desc-block"><div class="pd-label">Описание:</div><div class="pd-desc-text">${item.name ||
                'Товар без описания'}</div></div>
...
function setupCurrentOrder(cell, items, identifier, name, type) {
    // Логика вероятности оплаты:
    // 90% - все товары "Оплата при получении" (isPaid = false)
    // 5% - все товары "Оплачено" (isPaid = true)
    // 5% - смешанный статус (рандом для каждого товара)
    let paymentScenario = Math.random();
    let globalPaymentStatus = 0; 
    
    if (paymentScenario > 0.90 && paymentScenario <= 0.95) {
        globalPaymentStatus = 1; 
    } else if (paymentScenario > 0.95) {
        globalPaymentStatus = 2; 
    }
    ...
    let finalItems = items.map(i => {
        ...
        if (type === 'courier') {
            ...
            if (globalPaymentStatus === 0) {
                isPaid = false; // Оплата при получении
            } else if (globalPaymentStatus === 1) {
                isPaid = true; // Оплачено
            } else {
                isPaid = Math.random() < 0.5; // Смешанный вариант
            }
        }
...
function switchToOrder(orderId) {
    ...
    // Проверка 18+ теперь только если это не курьер
    if (hasAdult && !order.ageVerified && order.type !== 'courier') {
        currentOrder = order;
        ...
        noBtn.onclick = () => {
            toggleModal('age-verification-modal', false);
            // Удаляем товары 18+ только из текущего заказа выдачи (order.items).
            // В базе (userData.receptionItems) они остаются, и их можно будет выдать потом.
            order.items = order.items.filter(i => !i.isAdult);
            if (order.items.length === 0) {
                finishOrderSession(orderId);
                showAlert('В заказе остались только товары 18+. Заказ отменен, но товары остались в базе.', false);
                showMainScreen();
            } else {
                saveActiveOrders();
                proceedWithOrder(orderId);
        ...
        // Если курьер или уже проверено, пропускаем модалку
        proceedWithOrder(orderId);
    }
}
...
function renderPreVerificationContent(justScannedCode = null) {
    ...
    verifiedItems.slice().reverse().forEach(item => {
        ...
        const codeClass = item.isAdult ? 'product-code adult-blur-text' : 'product-code';
        ...
        <div class="pv-item-card verified${lostClass}">
             ...
             <div class="pv-item-image">
                ${imageContent}
                ${!item.isAdult ? `<img src="plus.svg" class="more-details-btn" data-code="${item.code}" title="Подробнее">` : ''}
            </div>
...
'Товар'}</div>
    ...
    unverifiedContainer.querySelectorAll('.skip-one-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            'notfound') { item.isVerified = true; item.isLost = true; } 
...
function preVerifyItemByInput() {
    ...
    // ИЗМЕНЕНО: Проверяем ровно 10 цифр
    if (code.length !== 10) {
        playCustomSound('failScan.mp3');
        showAlert('Штрихкод должен содержать ровно 10 цифр!', true);
        return;
    }
...
function checkAllVerified() {
    ...
    if (isLostItemsVerificationMode) {
        if (currentOrder.items.filter(i => i.isLost).length === 0) {
            isLostItemsVerificationMode = false;
            showAlert("Все утерянные товары найдены!", false); showOrderViewScreen();
        } return;
    }
    if (currentOrder.items.every(item => item.isVerified)) {
        ...
        if (currentOrder.type === 'courier' && lostCount > 0) {
            if (lostCount === currentOrder.items.length) {
                // Утеряны ВСЕ товары
                toggleModal('courier-all-lost-modal', true);
            } else {
                // Утеряна только часть товаров
                document.getElementById('courier-some-lost-title').textContent = `Товары не найдены: ${lostCount}`;
                renderCourierLostItemsList();
...
function renderCourierLostItemsList() {
    const list = document.getElementById('courier-lost-items-list');
    list.innerHTML = '';
    const lostItems = currentOrder.items.filter(i => i.isLost);
    
    lostItems.forEach(item => {
        const codeStr = String(item.code);
        const part1 = codeStr.length >= 5 ? codeStr.slice(0, 5) : codeStr;
        const part2 = codeStr.length > 5 ? codeStr.slice(5) : '';
        const randomBarcode = Math.floor(1000000000 + Math.random() * 9000000000).toString(); 
        
        const imageContent = item.isAdult ? 'adult.svg' : (item.image || 'https://placehold.co/100/png?text=Box');
     
         
        const div = document.createElement('div');
        div.className = 'lost-item-card-new';
        div.innerHTML = `
            <img src="${imageContent}" class="lost-item-img-new">
            <div class="lost-item-info-new">
                <div class="lost-item-name">${item.name || 'Товар'}</div>
              
                <div class="lost-item-cell">Ячейка: ${formatCellDisplay(item.cell || 
                currentOrder.cell)}</div>
                <div class="lost-item-code">ШК: <span class="gray-part">${part1}</span> <span class="black-part">${part2}</span></div>
                <div class="lost-item-attr">Размер: One Size, Цвет: Не указан</div>
                <div class="lost-item-barcode">Баркод: ${randomBarcode}</div>
                <button 
class="lost-item-btn-new" data-code="${item.code}">ШК поврежден</button>
            </div>
    ...
    list.querySelectorAll('.lost-item-btn-new').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (item) {
               ...
               true;
                item.isVerified = true;
                item.isNoBarcode = true;
                saveActiveOrders();
                
                const 
newLostCount = currentOrder.items.filter(i => i.isLost).length;
               ...
               document.getElementById('courier-some-lost-title').textContent = `Товары не найдены: ${newLostCount}`;
...
function renderProductCards() {
    ...
    currentOrder.items.forEach(item => {
        ...
        // Визуальное состояние товара напрямую зависит от чекбокса
        const isForIssue = item.isChecked; 
        ...
        // Чекбокс всегда активен, чтобы товар можно было вернуть в примерку
        const checkboxDisabled = '';
        
        let paymentBadge = '';
        if (!isCourier) {
            paymentBadge = item.isPaid ? `<span class="payment-badge paid">Оплачено</span>` : `<span class="payment-badge unpaid">Не оплачено</span>`;
        }

        // Плашка "Отказ", если галочка снята
        let returnBadgeHtml = '';
        if (!isForIssue) {
            returnBadgeHtml = `<div class="return-status-badge">Отказ</div>`;
        }
        ...
        const codeDisplayFinal = item.isAdult ? `<span class="adult-blur-text">${codeDisplay}</span>` : codeDisplay;
        ...
        `;
        container.appendChild(card);
        card.querySelector(`#check-${item.code}`).addEventListener('change', (e) => toggleItemSelection(e, item.code));
    });
}
...
function updateRecommendedPackages() {
    ...
    // Показывать рекомендацию ТОЛЬКО для курьера и если есть товары
    ...
    if (itemsCount >= 1 && itemsCount <= 3) {
        pkgCount = 1;
    } else if (itemsCount >= 4 && itemsCount <= 5) {
        pkgCount = 2;
    } else if (itemsCount >= 6 && itemsCount <= 8) {
        pkgCount = 3;
    } else {
        pkgCount = Math.ceil(itemsCount / 3);
    }

    // Рандомизация типа пакета в зависимости от кол-ва товаров
    let pkgType = 'Пакет-майка';
    const rand = Math.random();
    
    if (itemsCount <= 3) {
        pkgType = rand > 0.8 ?
'Пакет средний' : 'Пакет-майка';
    } else if (itemsCount <= 6) {
        if (rand > 0.7) pkgType = 'Пакет большой';
        else if (rand > 0.3) pkgType = 'Пакет средний';
        else pkgType = 'Пакет-майка';
    } else {
        pkgType = rand > 0.6 ?
'Пакет большой' : 'Пакет средний';
    }

    const recText = `${pkgType} — ${pkgCount} шт.`;
    container.innerHTML = `
                ...
                <div class="client-layout">
                    ...
                    <hr class="full-hr">
                     ...
                     <div class="payment-block">
                      ...
                      <span id="payment-details-text">${isPaymentDetailsOpen ?
                         'Свернуть' : 'Подробнее'}</span>
                            <i id="payment-details-icon" class="fas fa-chevron-${isPaymentDetailsOpen ? 'up' : 'down'}"></i>
                        </div>

                        <div class="payment-details-content" id="payment-details-content" style="display: ${isPaymentDetailsOpen ? 
'flex' : 'none'};">
    ...
    <span id="payment-details-method">${methodText}</span>
                            ...
                            <hr style="border: 0;
border-top: 1px solid var(--border-color); margin: 5px 0;">
    ...
    if (issueBtn) {
        if (currentOrder.type === 'courier') {
            ...
            if (currentOrder.items.length === 0) {
                ...
                issueBtn.textContent = hasUnpaidSelected ?
"Оплатить и выдать" : "Выдать";
            }
            issueBtn.disabled = false;
            issueBtn.style.opacity = '1';
            issueBtn.style.cursor = 'pointer';
    ...
    if (selectAllCheckbox) {
        const validItems = itemsToShow.filter(i => !i.isLost);
        if (validItems.length > 0) {
            const allChecked = validItems.every(i => i.isChecked);
            const someChecked = validItems.some(i => i.isChecked);
            
            // Если все выбраны — горит галочка. Если часть — горит минус (indeterminate). Если все отказ — пусто.
...
function updatePaymentDetails() {
    ...
    const totalSum = checkedUnpaidItems.reduce((sum, item) => sum + (item.price || 0), 0);
    const discount = checkedUnpaidItems.length > 0 ?
    (currentOrder.paymentDetails.discount || 0) : 0;
...
function toggleItemSelection(event, code) {
    ...
    if (item) {
        // Если пытаются отказаться от невозвратного товара
        if (!event.target.checked && !item.isReturnable) {
            showAlert('Этот товар невозвратный. Отказ невозможен.', true);
            playErrorSound();
            event.target.checked = true; // Возвращаем галочку принудительно
            return;
        }
        ...
        if (!item.isChecked) {
            ...
            // Опционально воспроизвести звук отказа, если он настроен
    ...
    saveActiveOrders(); 
    renderProductCards(); // Мгновенно перерисовываем, чтобы применились красные стили
    updateUIData();
}
...
function toggleSelectAll(event) {
    ...
    // Если до клика чекбокс был indeterminate, браузер может передать false. 
    // Наша логика: если есть хоть один невыбранный, мы хотим выбрать ВСЕ.
    const validItems = currentOrder.items.filter(i => !i.isLost);
    const allCheckedBefore = validItems.every(i => i.isChecked);
    
    // Переопределяем логику: клик снимает всё ТОЛЬКО если до этого было выбрано абсолютно всё.
    // Во всех остальных случаях клик должен включить (выбрать) всё.
    const shouldCheckAll = !allCheckedBefore; 
    
    // Если нужно снять галочки, обновляем состояние event.target чтобы UI не "прыгал"
    if (!shouldCheckAll) {
    ...
    currentOrder.items.forEach(i => { 
        if (!i.isLost) {
            // Проверка на невозвратный товар при массовом отказе
            if (!shouldCheckAll && !i.isReturnable) {
                if (!errorShown) {
                    showAlert('Некоторые товары невозвратные, от них нельзя отказаться.', true);
                    playErrorSound();
                    errorShown = true;
                }
                return; // Игнорируем невозвратные
            }
            
            i.isChecked = shouldCheckAll;
            i.status = shouldCheckAll ? 'selected' : 'returned';
        } 
    });
...
function removeItemFromFittingBySearch() {
    ...
    const code = input.value.trim();
    if (code.length !== 10) return; // ИЗМЕНЕНО: было 9, теперь 10 цифр
    const items = currentOrder.items;
    
    // ИЗМЕНЕНО: Ищем любой первый попавшийся ВОЗВРАТНЫЙ товар на примерке
    ...
    if (itemToReturnIndex !== -1) {
        ...
        // ИЗМЕНЕНО: Убираем из текущего места в списке и закидываем в самое начало
        ...
        showAlert('В заказе нет доступных товаров для возврата.', true);
        playErrorSound(); 
    }
    input.value = ''; 
    input.focus();
...
function showSuccessScreen(issuedItems, returnedItems) {
    ...
    if (currentOrder.type === 'courier') {
        ratingBlock.style.display = 'block'; nextClientBtn.style.display = 'none';
...
function openRestoreItemsModal(items) {
    ...
    items.forEach((item, index) => {
        ...
        div.innerHTML = `<div class="hc-left"><div class="checkbox-wrapper-purple"><input type="checkbox" id="restore-check-${index}" class="restore-checkbox" checked></div></div><div class="hc-image"><div class="product-image-mid">${imageContent}</div></div><div class="hc-info"><div class="${codeClass}">${item.code}</div><div 
        class="${nameClass}">${item.name || 'Товар'}</div></div>`;
...
// ВНИМАНИЕ: Исправлена функция toggleModal - теперь оверлей не затрагивается при открытии help-menu
function toggleModal(modalId, show) {
    ...
    if (show) {
        if (modalId === 'help-menu') { 
            // Только показываем само меню, оверлей не трогаем!
