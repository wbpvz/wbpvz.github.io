...
function formatCellDisplay(cell) {
    ...
    if (cellStr.includes(',')) return cellStr.split(',').map(c => `<div style="margin-bottom: 5px; line-height: 1; font-weight: inherit;">${parseInt(c.trim(), 10)}</div>`).join('');
    return `<div style="margin-bottom: 5px; line-height: 1; font-weight: inherit;">${parseInt(cellStr, 10)}</div>`;
}
    ...
    container.innerHTML = `
        <div class="pd-layout">
            ...
            <div class="pd-info-col">
                ...
                <div class="pd-price-row">${(item.price || 0).toFixed(2)} Br</div>
...
function processAndFindOrder(orderId) {
    ...
    if (isCourier) {
        type = 'courier';
        name = `Курьер ID: ${Math.floor(10000000000 + Math.random() * 90000000000).toString()}`;
...
function setupCurrentOrder(cell, items, identifier, name, type) {
    let discount = 0;
    if (type !== 'courier') {
        discount = Number((Math.random() * 9 + 1).toFixed(2));
    }
    ...
    currentOrder = {
        ...
        paymentDetails: { method: method, discount: discount, total: rawTotal }
    };
}
...
function renderPreVerificationContent(justScannedCode = null) {
    ...
    if (isLostItemsVerificationMode) {
        ...
        document.getElementById('pv-header-text').innerHTML = `Отсканируйте утерянные: <strong>${totalLostInitially - currentlyLost}</strong> из <strong>${totalLostInitially}</strong>`;
    } else {
        const totalCount = itemsToProcess.length;
        const verifiedCountTotal = itemsToProcess.filter(i => i.isVerified).length;
        document.getElementById('pv-header-text').innerHTML = `Отсканируйте товары перед примеркой: <strong>${verifiedCountTotal}</strong> из <strong>${totalCount}</strong>`;
    }
    ...
    if (isCourier) {
        ...
        const courierTotal = currentOrder.items.length;
        const courierVerified = currentOrder.items.filter(i => i.isVerified).length;
        document.getElementById('pv-courier-verified-val').textContent = `${courierVerified} из ${courierTotal}`;
    ...
    const initialScanImg = document.getElementById('pv-initial-scan-img');
    if (initialScanImg) {
        initialScanImg.style.display = verifiedItems.length > 0 ? 'none' : 'block';
    }

    const hasUnpaid = currentOrder.items.some(i => !i.isPaid && !i.isLost);
    const unpaidWarning = document.getElementById('pv-unpaid-warning');
    if (unpaidWarning) {
        unpaidWarning.style.display = (hasUnpaid && !isCourier && !isLostItemsVerificationMode) ? 'block' : 'none';
    }
    ...
    if (justScannedCode) {
        ...
        <div class="pv-scan-success-message">
            ...
            <div style="font-weight: 500; font-size: 0.95rem; line-height: 1.4;">
                <span style="display: block; font-weight: bold; margin-bottom: 2px;">Товар отсканирован!</span>
                <span>Отсканируйте следующий товар.</span>
            </div>
        </div>`;
    }
...
function renderCourierBagsWarning() {
    const container = document.getElementById('courier-bags-warning-container');
    if (!container || !currentOrder) return;
    if (currentOrder.type === 'courier') {
        const count = currentOrder.items.length;
        let bags = count <= 5 ? 1 : count <= 10 ? 2 : Math.ceil((count - 10) / 10) + 2;
        const sizes = ["Малый", "Средний", "Большой"];
        const size = sizes[Math.floor(Math.random() * sizes.length)];
        container.innerHTML = `<div class="bags-warning-alert"><i class="fas fa-info-circle"></i> Рекомендуемое количество пакетов: ${size} ${bags} шт.</div>`;
...
function showOrderViewScreen() {
    ...
    const dividers = document.querySelectorAll('.info-divider');
    ...
    if (isCourier) { 
        ...
        if(packagesBlock) packagesBlock.style.display = 'flex'; 
...
function renderProductCards() {
    ...
    currentOrder.items.forEach(item => {
        ...
        const imageContent = item.isAdult ? `<img src="adult.svg" style="width:100%; height:100%;">` : (item.image ? `<img src="${item.image}" style="width:100%; height:100%; object-fit:contain;">` : '<span style="font-size:2rem">📦</span>');
...
function updateUIData() {
    ...
    if (currentOrder.type === 'courier') {
        let courierId = (currentOrder.name.match(/ID: (\d+)/) || [])[1];
        if (!courierId || courierId.length < 11) {
            courierId = Math.floor(10000000000 + Math.random() * 90000000000).toString();
            currentOrder.name = `Курьер ID: ${courierId}`;
        }
        document.getElementById('display-identifier').innerHTML = `<div class="info-label" style="font-size:0.8rem!important; text-align:right; color:var(--text-secondary-color);">Курьер ID</div><div style="font-size: 1.1rem; font-weight: bold; text-align:right;">${courierId}</div>`;
        document.getElementById('issue-btn').textContent = "Выдать";
    } else {
        const lastFourDigits = currentOrder.identifier.split('-')[0];
        const clientName = `<span style="color:var(--text-secondary-color); font-size:0.9rem;">+375 (••) ••• <b style="color:var(--text-primary-color); font-size:0.9rem;">${lastFourDigits.slice(0, 2)} ${lastFourDigits.slice(2, 4)}</b></span>`;
        document.getElementById('display-identifier').innerHTML = `<div class="info-label" style="font-size:0.8rem!important; text-align:right; color:var(--text-secondary-color);">Клиент</div><div style="text-align:right;">${clientName}</div>`;
        const hasUnpaidSelected = checkedItems.some(i => !i.isPaid);
        document.getElementById('issue-btn').textContent = hasUnpaidSelected ? "Оплатить и выдать" : "Выдать";
    }
    ...
    let discount = checkedUnpaidItems.length > 0 ? (currentOrder.paymentDetails.discount || 0) : 0;
    if (discount > totalSum) discount = totalSum;
    
    let finalSum = totalSum - discount;
    if (finalSum < 0 || checkedUnpaidItems.length === 0) finalSum = 0; 
    document.getElementById('total-price-value').textContent = `${finalSum.toFixed(2)} Br`;
...
function updatePaymentDetails() {
    ...
    if (selectedItems.length === 0) {
        ...
        methodText = "При получении";
    } else {
        methodText = "Сразу, При получении";
    }

    document.getElementById('payment-details-method').textContent = methodText;
    document.getElementById('payment-details-total').textContent = `${totalSum.toFixed(2)} Br`;
    document.getElementById('payment-details-discount').textContent = `${discount.toFixed(2)} Br`;
    document.getElementById('payment-details-final').textContent = `${final.toFixed(2)} Br`;
}
    ...
    packages.forEach(pkg => {
        ...
        row.innerHTML = `
            ...
            <div class="pkg-name">
                ...
                <div style="font-size:0.9rem; color:grey;">${pkg.price} Br</div>
            </div>
...
function handleIssueClick() {
    ...
    if (currentOrder.type === 'courier') {
        ...
        playCustomSound('tell_code_to_courier.mp3');
        return;
    }
    processIssue();
}
...
function showSuccessScreen(issuedItems, returnedItems) {
    ...
    const uniqueCells = [...new Set(currentOrder.items.map(i => i.cell))];
    let cellDisplay = uniqueCells.length > 0 ? uniqueCells[0] : '-';
    if (uniqueCells.length > 1) cellDisplay += '<br><span style="font-size:0.8em; color:var(--text-secondary-color);">другие</span>';
    document.getElementById('ss-cell-number').innerHTML = cellDisplay;
    ...
    if (currentOrder.type === 'courier') {
        ...
        document.getElementById('ss-client-name').innerHTML = `<strong style="font-size:1.1rem;">${courierId}</strong>`;
        document.getElementById('ss-title-label').textContent = 'Курьер ID:';
        document.getElementById('ss-returned-label').textContent = 'Не найдено:';
        ...
        document.getElementById('ss-title-label').textContent = 'Клиент';
        document.getElementById('ss-returned-label').textContent = 'Отмена:';
    ...
    document.getElementById('success-screen-title').textContent = "Выдача заказа завершена";
    ...
    if (currentOrder.type === 'courier') {
        ...
        document.getElementById('ss-submit-rating-btn').disabled = true;
    } else {
...
function renderActiveOrders() {
    ...
    allActiveOrders.forEach(order => {
        ...
        const verifiedClass = order.isVerified ? 'verified-compact' : '';
        orderEl.className = `active-order ${currentOrder && order.identifier === currentOrder.identifier ? 'highlighted' : ''} ${verifiedClass}`;
        ...
        // Маленькая ячейка с количеством товаров сверху (степень)
        const cellText = String(displayCell).replace(/,/g, ', ');
        orderEl.innerHTML = `<div class="order-info-wrapper">${fromReturnIcon}<span class="order-cell" style="display:flex; align-items:flex-start; line-height:1;"><span style="font-size:1rem; font-weight:bold;">${cellText}</span><sup style="font-size:0.75rem; margin-left:2px; font-weight:bold; color:var(--text-secondary-color);">${order.items.length}</sup></span></div>`;
...
function updateSuccessScreenStars(rating) {
    ...
    if(rating > 0) document.getElementById('ss-submit-rating-btn').disabled = false;
}
