...
function createReturnItemCardElement(item) {
    ...
    const defectiveOverlay = item.isDefective ?
        `<div class="defective-ribbon"><span>БРАК</span></div>` : '';
...
function openReturnProductDetails(item) {
    ...
    let receptionDateStr = item.date ?
        new Date(item.date).toLocaleString('ru-RU') : "Неизвестно";
...
function setupCustomerReturnFlow() {
    ...
    document.getElementById('cr-step1-next-btn').addEventListener('click', () => {
        ...
        if (!/^\d{6}$/.test(val)) {
            showAlert("Ошибка: Введите корректный код (6 цифр)!", true);
            input.focus();
            return;
        }
    ...
    document.getElementById('cr-step2-next-btn').addEventListener('click', () => {
        ...
        // ВАЛИДАЦИЯ через showAlert
        if (!/^\d{10}$/.test(val)) {
             showAlert("Ошибка: ШК товара должен состоять из 10 цифр!", true);
             input.focus();
             return;
        }
...
function showStep3() {
    ...
    const imageContent = crSelectedItem.isAdult ?
        `<img src="adult.svg" style="width:120px; height:120px; object-fit:contain;">` : `<img src="${crSelectedItem.image || 'https://placehold.co/200x200'}" style="width:120px; height:120px; object-fit:contain;">`;
...
function handleStickerInput() {
    ...
    if (!/^\d{10}$/.test(val)) {
        showAlert('Ошибка: Введите корректный 10-значный код стикера', true);
        input.focus();
        return;
    }
        ...
        <div class="fullscreen-modal" id="return-sticker-modal">
            <div class="modal-content">
                ...
                <div class="input-group">
                    <input type="text" id="return-sticker-input" placeholder="10 цифр..." maxlength="10" inputmode="numeric">
                </div>
