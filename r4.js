...
function renderDisposalItems() {
    ...
    disposeAllBtn.onclick = () => {
        ...
        if (!modal) { // Запасной вариант, если модальное окно не найдено
            if (confirm(`Вы уверены, что хотите удалить из ПО ${itemsToDispose.length} товар(ов)? Это действие необратимо.`)) {
                disposeAllItems(itemsToDispose);
            }
            return;
        }
        
        document.getElementById('disposal-confirm-text').textContent = `Вы уверены, что хотите удалить товары из ПО в количестве: ${itemsToDispose.length}?`;
...
function disposeAllItems(itemsToDispose) {
    ...
    showAlert(`Все ${itemsToDispose.length} товар(ов) удалены из ПО`, false);
    renderDisposalItems();
}