...
function setupMobileEventListeners() {
    ...
    // ==========================================
    // ЛОГИКА РЕАЛЬНОГО СКАНИРОВАНИЯ С КАМЕРЫ
    // ==========================================
    ...
    if(scanBtn && camOverlay) {
        scanBtn.addEventListener('click', () => {
            ...
            // Запускаем камеру с квадратной областью сканирования
            html5QrCode.start(
                { facingMode: "environment" }, // задняя камера
                {
                    fps: 10,
                    qrbox: { width: 250, height: 250 } // Строго КВАДРАТНАЯ рамка поиска
                },
                (decodedText) => {
                    // Если результат состоит строго из 9 цифр
                    if (/^\d{9}$/.test(decodedText)) {
                        // Останавливаем сканер
                        html5QrCode.stop().then(() => {
                            camOverlay.style.display = 'none';
                            playScannerSound();
                            // Пускаем товар в обработку (сразу покажет экран успеха)
                ...
                (errorMessage) => {
                    // Игнорируем шумы, пока не найдем нужный код
                }
            ).catch((err) => {
    ...
    // Кнопка "Сканировать еще" на экране успеха
    document.getElementById('m-res-next-btn')?.addEventListener('click', () => {
        // Прячем экран результата
        document.getElementById('m-result-screen').style.display = 'none';
        // Автоматически нажимаем невидимую кнопку старта, чтобы сразу открыть камеру
