...
function handleScanSubmit() {
    ...
    if (!code) {
        showAlert('Отсканируйте или введите цифры ШК', true);
        return;
    }
    ...
    if (!scannedItem) {
        showAlert('Товар с таким ШК не найден на складе', true);
        return;
    }
...
function handleMethodNextClick() {
    ...
    if (!suggested && !manual) {
        showAlert('Пожалуйста, выберите способ смены ячейки', true);
        return;
    }
...
function populateSuggestedCells(count) {
    ...
    if (emptyCells.length === 0) {
        container.innerHTML = '<p style="font-size: 0.9rem; color: var(--text-secondary-color);">Нет свободных ячеек на складе</p>';
        return;
    }
...
function handleManualCellSubmit() {
    ...
    if (!isCellEmpty(paddedCell)) {
        showAlert('Ячейка занята! Выберите другую.', true);
        return;
    }
