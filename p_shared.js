...
function attachSharedEventListeners() {
    ...
    if (sendFeedbackBtn) {
        sendFeedbackBtn.addEventListener('click', () => {
            ...
            // Здесь можно вызвать кастомный alert, но по вашему заданию успешные убраны,
            // поэтому оставим просто закрытие окна и сброс состояния.
...
async function saveEmployeeData() {
    ...
    try {
        ...
        // showAlert('Сотрудник сохранен', false); // Убрано успешное уведомление
...
async function executeDeleteEmployee(empId) {
    ...
    try {
        ...
        if (snap.exists()) {
            const emps = Object.values(snap.val());
            const empToDelete = emps.find(e => String(e.id) === String(empId));
            
            if (empToDelete && empToDelete.role === 'owner') {
                const ownerCount = emps.filter(e => e.role === 'owner').length;
                if (ownerCount <= 1) {
                    showAlert('Ошибка: Нельзя удалить последнего собственника ПВЗ. Сначала назначьте другого.', true);
                    return; 
                }
            }
        }
        ...
        // showAlert('Сотрудник удален', false); // Убрано успешное уведомление
        loadEmployeesToModal();
    } catch (e) {
        showAlert('Ошибка удаления: ' + e.message, true);
    }
}
...
/* ИСПРАВЛЕНИЕ: Выводить только ошибки, прямоугольное со слегка скругленными углами */
function showAlert(message, isError = false) {
    ...
    alert.innerHTML = `<i class="fas fa-times-circle" style="color: var(--error); font-size: 1.5rem;"></i><span>${message}</span>`;
